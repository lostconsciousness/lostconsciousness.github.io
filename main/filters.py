...
def all_fluid_volume():
    ...
    sort = []
    for pod in allObjects:
        if pod.fluid_volume.split(' ')[0] not in pp and pod.fluid_volume != "-"and pod.fluid_volume != "":
            pp.append(pod.fluid_volume.split(' ')[0])
            povtor.append(float(pod.fluid_volume.split(' ')[0]))
    sort = sorted(povtor)
    for pod in sort:
        res = []
        strpod = str(pod)
...
class PodFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Podik
        fields = ({'price', 'categoryId', 'param', 'nicotine_strength', 'fluid_volume', 'resistance', 'power', 'atomizer_volume', 'max_power', 'puffs_number'})
